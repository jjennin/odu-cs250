...
bool Shipment::comparing(Shipment name){
    if(name.shipmentrecieved.year > shipmentrecieved.year){
        return true; // if year greater, return true
    }
    else if(name.shipmentrecieved.year < shipmentrecieved.year){
        return false; // if year less, return false
    }
    if(name.shipmentrecieved.month > shipmentrecieved.month){
        return true; // if month greater, return true
    }
    else if(name.shipmentrecieved.month < shipmentrecieved.month){
        return false; // if month less, return false
    }
    if(name.shipmentrecieved.day > shipmentrecieved.day){
        return true; // if day greater, return true
    }
    else if(name.shipmentrecieved.day < shipmentrecieved.day){
        return false; // if day less, return false
    }
    /// EXPDATE TIE BREAKER
    if(name.expdate.year > expdate.year){
        return true; //  if year greater, return true
    }
    else if(name.expdate.year < expdate.year){
        return false; // if year less, return false
    }
    if(name.expdate.month > expdate.month){
        return true; // if month greater, return true
    }
    else if(name.expdate.month < expdate.month){
        return false; // if month less, return false
    }
    if(name.expdate.day > expdate.day){
        return true; // if day greater, return true
    }
    else if(name.expdate.day < expdate.day){
        return false; // if day less, return false
    }
}
...
void sortShipmentOrder(Shipment* shipmentList, int totalShipments){
    for(int i = 0; i < totalShipments; ++i){ /// LOOP 1
        for(int j = 0; j < totalShipments; ++j){ /// LOOP 2 inside of 1
            ...
            if(shipmentList[i].comparing(shipmentList[j])){ /// COMPARES SHIPMENT 1 to SHIPMENT 2
